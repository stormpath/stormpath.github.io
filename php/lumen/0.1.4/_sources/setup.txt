...
.. note::
    One item to note is for new accounts at Stormpath. If you created a new account and tenant
    for this project, you will need to click on the ID Site tab. There is nothing to do in
    the tab, it will just initialize some values that we will need when booting lumen.
...
left to do before we can dive into the code is install the `Stormpath-Lumen`_
package from `Packagist`_.

To install Stormpath-Lumen, you'll need ``composer``.  Open your composer.json file
...
.. note::
    There is a known issue with running ``composer require stormpath/lumen`` with
    our guzzle package.

.. note::
    Stormpath-Lumen requires the following;

    Lumen 5.2.*, and PHP 5.5+.
...
.. _Stormpath-Lumen: https://www.packgist.org/stormpath/stormpath-lumen
