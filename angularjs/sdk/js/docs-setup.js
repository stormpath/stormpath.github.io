NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "$currentuser $http $notloggedin $scope $user access account accountdata address angular api attempt attempts authenticated authenticating boolean called case catch changed checks config console consume control controller create created current currently currentuser data delivered directory eh email emailed emitted enabled endpoint error events exist expired failure false fields follow format function generate givenname http invalid json literal log logged login meet message method methods_get modified module myapp myappctrl network null object observe occurs operation option passed passing password passwordresetrequest post posted proceed promise properties_register_uri property re-sends ready register_uri rejected representing required requirements requires resend resendverificationemail reset resetpassword resets resolve resolved response retains returns rules service sptoken status stormpath stormpath_config strength submitting success successful surname token triggers true type unknown unverified user username userservice valid var verification verified verifies verify verifypasswordresettoken weak wishes workflow wrapped"
    },
    {
      "section": "api",
    ...
    },
    {
      ...
      "keywords": "access account accounts add agnostic angular api application applications apps assumes attacks attempts authenticate authenticate-an-account authenticating authentication authentication_endpoint back-end based best blog browser changed choice client communicate complete consume cookie cookie-based cookies create create-an-account creation currently description directly docs email endpoint endpoints ensure environment environments expected expiration expire express find flag flow forgot form front-end generate guide heard http http-only https identifies implement insecure integrate integrated integration issue issued issuing javascript js json jwt jwts language logged-in logs manage management manually methods modify modifying module node oauth object overview password place policy post production properties_authentication_endpoint protect protected read refresh registration remove reset reset-an-accounts-password respond rest revocation revoked sdk sdks secondary secure secure-only sense server session session-related sessions set setup single sp statement storage store stored stormpath stormpath_config strategy support system table token tokens triggering truth ttl type unauthorized urls user vague valid verification verify verify-an-email-address visit work workflow works xss"
    }
  ],
  "apis": {
    "api": true,
