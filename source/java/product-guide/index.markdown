...
- [offset](#collections-offset): The zero-based starting index in the entire collection of the first item to return. Default is `0`.
- [limit](#collections-limit): The maximum number of collection items to return for a single request. Minimum value is `1`. Maximum value is `100`. Default is `25`.
...
OrganizationCriteria criteria = Organizations.where(Organizations.name().eqIgnoreCase("Finance Organization"));
Organization organization = tenant.getOrganizations(criteria).single(); // single() will throw if this list contains zero or more than one element
String organizationHref = organization.getHref();
...
The following attributes are required and must be unique within your `Tenant`:
...
Organization organization = client.instantiate(Organization)
    ...
    .setDescription("The organization description goes here")
    .setNameKey("Org125")
...
A request to the `get` method of the `Organization` resource class, or to the Client data store, returns a representation of an organization resource that includes the attributes.

If you don’t have the organization’s URL, you can find it by looking it up in the Stormpath Admin Console or by searching your tenant’s organizations (`tenant.getOrganizations()` for the organization and then using its href.

**Example Request**

String href = "https://api.stormpath.com/v1/organizations/kGoiwcorTTmkDDBsf02AbA";
Organization organization = client.getResource(href, Organization.class);   
...
organization.setName("New Organization Name")
    .setDescription("New Organization Description")
    .setNameKey("Finace2940")
    .setStatus(OrganizationStatus.DISABLED)
    .save();

Organizations have an “status” which defines its state in the systems: enabled and disabled. In the previous example the organization resource has been marked as `disabled`. A `disabled` organization cannot be used to store `Directories` or `Groups`.
...
Directory directory = getDirectoryToUseAsAccountStore();

String href = "https://api.stormpath.com/v1/organizations/Ls4iwcorTTmlxDBsf09dbA";
Organization organization = client.getResource(href, Organization.class);
